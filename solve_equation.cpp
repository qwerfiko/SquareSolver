...
NRoots solve_quadratic_equation (double a, double b, double c, double *x1, double *x2) {
    ...
    assert (isfinite (a) != 0); //TODO: assert на NAN
    assert (isfinite (b) != 0);
    assert (isfinite (c) != 0);
    ...
    if (is_equal_comparison (a, 0)) {   //< If a == 0, then the function for solving the linear equation solve_linear_equation (b, c, x1, x2) is called
        return solve_linear_equation (b, c, x1);    //< возвращется поле enum NRoots
    }
    ...
    else if (is_equal_comparison (discr, 0)) {   //< Handling case d == 0
        *x1 = -b / (2 * a);
        return ZERO_ONE;
    }

    else { //< Handling case d > 0
...
NRoots solve_linear_equation (double b, double c, double *x1) {
    ...
    else if (is_equal_comparison (b, 0) && !is_equal_comparison (c, 0)) {  //< Handling the case when b = 0, c != 0
        return INCORRECT;
    }

    else { //< Handling the case when b = 0, c != 0is_equal_comparison (b, 0) && is_equal_comparison (c, 0)
        return INF_ROOTS;
    }
}
